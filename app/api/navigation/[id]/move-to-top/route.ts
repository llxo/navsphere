...
export async function POST(
  ...
  try {
    ...
    // 如果已经在顶部，不需要移动
    if (itemIndex === 0) {
      return NextResponse.json({ success: true, message: 'Item is already at the top' })
    }

    // 移动到顶部
    const [item] = data.navigationItems.splice(itemIndex, 1)
    data.navigationItems.unshift(item)

    // 保存更新后的数据
    await commitFile(
      'navsphere/content/navigation.json',
      JSON.stringify(data, null, 2),
      `Move navigation item "${item.title}" to top`,
      session.user.accessToken
    )

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Failed to move navigation item to top:', error)
    return NextResponse.json(
      { 
        error: 'Failed to move navigation item to top', 
